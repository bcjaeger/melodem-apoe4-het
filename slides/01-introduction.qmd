---
title: "MELODEM data workshop"
subtitle: "Introduction"
author: "Byron C. Jaeger, PhD"
institute: "Wake Forest University School of Medicine"
format: 
  revealjs:
    slide-number: true
    footer: Slides available at <https://byronjaeger.com/talk>
    include-before-body: header.html
    include-after-body: footer-annotations.html
    theme: [default, tidymodels.scss]
    width: 1280
    height: 720
---

```{r setup, cache=FALSE}

knitr::opts_chunk$set(echo = FALSE, 
                      message = FALSE,
                      warning = FALSE,
                      dpi = 300,
                      cache = TRUE,
                      fig.height = 6,
                      fig.align = 'center')

```

::: r-fit-text
Welcome!
:::

::: columns
::: {.column width="50%"}

<center>

### <i class="fa fa-wifi"></i>

Wi-Fi network name

`TODO-ADD-LATER`

</center>

:::

::: {.column width="50%"}

<center>

### <i class="fa fa-key"></i>

Wi-Fi password

`TODO-ADD-LATER`

</center>

:::
:::

## Hello! My name is Byron 

:::{.column width="25%"}
I am an R enthusiast.

I love dogs.

I study risk <br/> prediction + machine learning
:::

:::{.column width="70%"}
![](img/run_R_kids.png){width=90%}
:::

## Schedule for day 1: Morning

*Introduction, data management (75m)*

- Break (15m)

*Decision trees and random forests (75m)*

- Break (15m)

**Lunch (1 hour)**

## Schedule for day 1: Afternoon

*Develop and evaluate prediction models (75m)*

- Break (15m)

*Develop and evaluate causal models (75m)*

- Break (15m)

**Collaboration (1 hour)**

## Schedule for day 2: Morning

*Review and discuss manuscript aims (30m)*

- Align on aims and responsibilities

*Three break-out sessions (60m each)*

- Work independently or in small groups (45m)
- Progress updates (10m)
- Break (5m)

**Wrap-up (30m)**

# Part 1: Introduction and data management

## Set-up R packages

Make sure we all have up-to-date versions of these R packages:

```{r}
#| label: installs
#| eval: false
#| echo: true

# Install the packages for the workshop
pkgs <- 
  c("bonsai", "doParallel", "embed", "finetune", "lightgbm", "lme4",
    "plumber", "probably", "ranger", "rpart", "rpart.plot", "rules",
    "splines2", "stacks", "text2vec", "textrecipes", "tidymodels", 
    "vetiver", "remotes")

install.packages(pkgs)
```

## Set-up GitHub

Make sure we all have GitHub account with personal access token (PAT) stored in Rstudio

1. Open Rstudio
1. Copy/paste code below, adjust `destdir`, and run

```{r, eval=FALSE, echo = TRUE}

library(usethis)

create_from_github("bcjaeger/melodem-apoe4-het",
                   destdir = "path/of/your/choice", 
                   fork = TRUE)

```

## Introducing `targets`

```{=html}
<iframe width="1280" height="550" src="https://books.ropensci.org/targets/" title="Webpage example"></iframe>
```

## Your turn

1. Open `_targets.R` in the `melodem-apoe4-het` project.
1. Run `tar_glimpse()` to inspect the pipeline.
1. Run `tar_make()` to make the pipeline.

Red sticky note if you encounter an error

Green sticky note when you have successfully made the pipeline

## Working together, separately.

Why do we need all the bells and whistles?

- Management of multiple datasets (`targets`)

- Coordinating analyses over these datasets (`targets`)

- Large amount of code from multiple authors (GitHub)

- Collaborative discussions in public makes better science (GitHub)

## Start with data management

In the `_targets.R` file: 

::: columns
::: {.column width="50%"}

```{r, eval = FALSE, echo = TRUE}

file_sim_1_tar <- tar_target(
  file_sim_1,
  command = "data/sim_1-raw.csv",
  format = 'file'
)

data_sim_1_tar <- tar_target(
  data_sim_1,
  data_prepare(file_sim_1, 
               age_range = c(55, 80))
)

```

:::

::: {.column width="50%"}

![](diagram/targets-vis-data_sim_1.png){width=90%}

:::
:::

This will be done with your data, too!

---

## Your turn 

We are going to add your data to the pipeline, carefully.

1. Think of a **name** and **label** for your data.

    + Example name: regards
    
    + Example label: Reasons for Geographic and Racial Differences in Stroke

2. Save a copy of your data in `data/sensitive`. The name of your file should be `name-raw.csv` or `name-raw.sas7bdat`, where `name` is your data's name. E.g., `regards-raw.csv`

## Your turn 

We are going to add your data to the pipeline, carefully.

3. Switch from the R console to the terminal.

4. Verify you have no uncommitted changes:

```{git, echo = TRUE, eval = FALSE}
git status
```

Should return "nothing to commit, working tree clean"

5. Create a new branch with git:

```{git, echo = TRUE, eval = FALSE}
git branch -b regards
```

## Your turn 

We are going to add your data to the pipeline, carefully.

6. Modify the code below in `_targets.R`, replacing `x` with the name of your data.

```{r, echo=TRUE, eval=FALSE}

# real data cohort

file_x_tar <- tar_target(               # <--- replace x with your data name
  file_x,                               # <--- replace x with your data name
  command = "data/sensitive/x-raw.csv", # <--- replace x with your data name
  format = 'file'
)

data_x_tar <- tar_target(         # <--- replace x with your data name
  data_x,                         # <--- replace x with your data name
  data_prepare(cohort_name = 'x') # <--- replace x with your data name
)

# don't forget to add these targets to the targets list!

```

7. run `tar_make()` in the R console. 

## Your turn

We are going to add your data to the pipeline, carefully.

You should now be able to load and print your data target by running:

```{r, echo=TRUE, eval=FALSE}

# Im using the simulated data, but you should use your own
tar_load(data_sim_1)

print(data_sim_1)

```

```{r}
withr::with_dir(
  here::here(),
  targets::tar_load(data_sim_1)
)

data_sim_1
```



