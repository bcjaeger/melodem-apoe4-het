...
1. Run `targets::tar_glimpse()` to inspect the pipeline.
1. Run `targets::tar_make()` to make the pipeline.
...
8. run `tar_load(data_x)`, where `x` is your data name, and print it
