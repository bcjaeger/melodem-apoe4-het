...
Suppose we're given $p$ predictors, each with $k$ unique values.
...
**big idea**: Interpret a model by evaluating its predictions on special data.
...
You can reduce the number of trees and increase terminal node size if you'd like to speed this process up
...
library(rsample)
set.seed(123)
# make the split: 75% training data, 25% testing
split <- initial_split(data = ad_data, prop = 3/4)
...
library(recipes)
vars_picked <- aorsf_select_vars(train, Class ~ .)

# Makes the recipe steps
recipe_steps <- recipe(data = train, Class ~ .) %>% 
  step_select(all_of(vars_picked)) %>% 
